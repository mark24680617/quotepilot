...
- ✅ 2026-07-12: **Security audit + hardening** (open-source, public API). Secret scan across full git history: CLEAN (no key/AK/password ever committed). Multi-agent audit (4 dims, adversarial verify) -> 14 confirmed findings, all fixed & redeployed + verified live: per-IP rate limits + global daily model-budget circuit breaker + input caps + bounded worker pool (credit drain); **owner-token** binding on edit/approve/reject (cross-user control); SSRF hardened (IPv4-mapped IPv6 + non-global metadata blocked); sandboxed-iframe artifact viewing (stored XSS); CORS locked to Pages origin; security headers; generic errors; artifact path containment. Redacted account-id/RAM-user + untracked .claude/. Docs: SECURITY.md. 40 tests green. **Residual (platform):** fcapp.run gateway reflects Origin — bounded by the caps. **User action needed:** set the Alibaba account to prepaid/no-auto-recharge + budget alert so $40 is a hard ceiling.
- ✅ 2026-07-12: **Settings auto-translate** — if a user fills only one language of a bilingual field, Save calls Qwen (qwen-max) to fill the missing side. Covers seller name/jurisdiction, payment/tax notes, catalog name/description/unit; **legal clauses are never auto-translated** (safety invariant). `POST /api/profile/save` (rate-limited + daily-gated); frontend refreshes the form + toasts how many gaps were filled. Verified live in-browser (emptied Name ZH → Save → 'AI filled 1 missing translation'). Also fixed: FC trigger now allows PUT (was gateway-blocked, which had silently broken the old PUT save path). 42 tests green.
