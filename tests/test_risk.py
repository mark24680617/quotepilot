...
def test_block_when_nothing_priceable():
    flags = risk.rule_flags(make_inquiry(), [], [], Decimal("0"), "hello")
...
def test_fapiao_detection_zh():
    flags = risk.rule_flags(
        make_inquiry(), [make_line()], [], Decimal("1000"), "请问能否开具增值税专用发票?"
    )
...
def test_wire_threshold():
    flags = risk.rule_flags(
        make_inquiry(), [make_line("60000.00")], [], Decimal("60000"), "big order"
    )
...
def test_discount_above_floor():
    ...
    flags = risk.rule_flags(inquiry, [make_line()], [], Decimal("1000"), "20% please")
...
def test_tight_deadline():
    ...
    flags = risk.rule_flags(inquiry, [make_line()], [], Decimal("1000"), "asap")
